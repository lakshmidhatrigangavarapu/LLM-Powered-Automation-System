...
This server now runs as an HTTP server on port 9001 using Server-Sent Events (SSE) for real-time communication.

### Endpoints

- **SSE Connection**: `GET http://localhost:9001/sse` - Establishes the SSE connection for receiving messages
- **Message Endpoint**: `POST http://localhost:9001/message?sessionId=<session-id>` - Sends messages to the server
